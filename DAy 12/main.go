...
func main() {
	...
	http.ListenAndServe("localhost:8050", route)
}
...
func loginPage(w http.ResponseWriter, r *http.Request) {
	...
	if len(fm) > 0 {
		...
		for _, fl := range fm {
			flashes = append(flashes, fl.(string))
		} // jadi apabila si flashnya memiliki nilai lebih dari 0 maka dia akan mengreturn message tersebut apabila ada isinya
	}
	Data.FlashData = strings.Join(flashes, "")
